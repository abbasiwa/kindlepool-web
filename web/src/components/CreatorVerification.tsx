    ...
    <Modal open={open} onClose={onClose} title="Creator Setup">
      {!profile ? (
        <div className="space-y-4 text-center py-4">
          <BadgeCheck size={48} className="mx-auto text-accent-primary" />
        ...
        <div className="space-y-4 text-center py-4">
          ...
          <p className="text-sm text-text-muted">Your creator profile is complete.</p>
        ...
        <div className="space-y-6">
            ...
            {stepLabels.map((label, i) => (
              <div key={label} className="flex items-center gap-2">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                  step > i + 1 ? 'bg-success text-text-inverse' : step === i + 1 ? 'bg-accent-primary text-text-inverse' : 'bg-surface-hover text-text-muted'
                }`}>
                  {step > i + 1 ? <Check size={14} /> : i + 1}
                </div>
                {i < totalSteps - 1 && <div className={`w-6 h-0.5 ${step > i + 1 ? 'bg-success' : 'bg-surface-hover'}`} />}
              </div>
            ))}
          </div>
            ...
            <motion.div key="s1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea value={bio} onChange={(e) => setBio(e.target.value)}
                  ...
                  className="w-full px-4 py-2.5 bg-surface border border-border-default rounded-xl text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent-primary/40 resize-none" />
              </div>
          ...
          {step === 2 && (
            <motion.div key="s2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
              ...
              <p className="text-sm text-text-muted">We'll send a verification code to your email.</p>
              ...
              <button onClick={() => setStep(3)} className="text-xs text-text-muted hover:text-text-primary mx-auto block">Skip →</button>
            </motion.div>
          )}
          ...
          {step === 3 && (
            <motion.div key="s3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
              ...
              <p className="text-sm text-text-muted">Link at least one social account to verify your identity.</p>
